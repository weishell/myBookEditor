...
import { isTimelineElement, normalizeTimelineAttrs } from './timeline-node';

/**
 * 时间轴块扩展：
 *  - isVoid：整块不可编辑，卡片内容由组件自绘（内部字段用局部 contentEditable）
 *  - normalizeNode：保证 attrs 合法、子节点为单个零宽文本，
 *    防止异常删除/变更破坏结构后渲染崩溃
 */
export const withTimeline = (editor: Editor) => {
  const { isVoid, normalizeNode } = editor;

  editor.isVoid = (element) => {
    if (isTimelineElement(element)) return true;
    return isVoid(element);
  };

  editor.normalizeNode = (entry: NodeEntry) => {
    const [node, path] = entry;
    if (isTimelineElement(node)) {
      const next = normalizeTimelineAttrs(node.attrs);
